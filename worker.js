		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					const 路径订阅类型 = 访问路径 === 'clash' ? 'clash' : '';
					const 用户客户端请求订阅 = 请求TOKEN === 订阅TOKEN || !!路径订阅类型;
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						const 订阅类型 = 路径订阅类型 || (isSubConverterRequest
							? 'mixed'
...
`.trim().split(/\n+/).map(line => {
	...
	const 创建固定香港节点文本 = (template, item, name) => {
		let nodeText = template || `  - {name: ${YAML字符串(name)}, server: ${item.ip}, port: ${item.port}, type: vless, uuid: ${YAML字符串(config_JSON?.UUID || '')}, tls: true, skip-cert-verify: false, servername: ${YAML字符串(config_JSON?.HOST || 'vpn.example.com')}, client-fingerprint: chrome, network: ws, ws-opts: {path: /, headers: {Host: ${YAML字符串(config_JSON?.HOST || 'vpn.example.com')}}}}`;
...
const MD5正弦表 = Array.from({ length: 64 }, (_, 索引) => Math.floor(Math.abs(Math.sin(索引 + 1)) * 0x100000000) | 0);

function MD5循环左移(数值, 位数) {
	return (数值 << 位数) | (数值 >>> (32 - 位数));
}

function MD5十六进制(文本) {
	const 输入字节 = new TextEncoder().encode(String(文本));
	const 原始长度 = 输入字节.length;
	const 填充长度 = (((原始长度 + 8) >> 6) + 1) << 6;
	const 填充字节 = new Uint8Array(填充长度);
	填充字节.set(输入字节);
	填充字节[原始长度] = 0x80;
	const 位长度低位 = (原始长度 * 8) >>> 0;
	const 位长度高位 = Math.floor(原始长度 / 0x20000000) >>> 0;
	for (let 偏移 = 0; 偏移 < 4; 偏移++) {
		填充字节[填充长度 - 8 + 偏移] = (位长度低位 >>> (8 * 偏移)) & 0xff;
		填充字节[填充长度 - 4 + 偏移] = (位长度高位 >>> (8 * 偏移)) & 0xff;
	}

	let 初始A = 0x67452301 | 0;
	let 初始B = 0xefcdab89 | 0;
	let 初始C = 0x98badcfe | 0;
	let 初始D = 0x10325476 | 0;
	const 分组 = new Int32Array(16);

	for (let 分组偏移 = 0; 分组偏移 < 填充长度; 分组偏移 += 64) {
		for (let 字索引 = 0; 字索引 < 16; 字索引++) {
			const 字节偏移 = 分组偏移 + 字索引 * 4;
			分组[字索引] = 填充字节[字节偏移] | (填充字节[字节偏移 + 1] << 8) | (填充字节[字节偏移 + 2] << 16) | (填充字节[字节偏移 + 3] << 24);
		}
		let 状态A = 初始A, 状态B = 初始B, 状态C = 初始C, 状态D = 初始D;
		for (let 轮次 = 0; 轮次 < 64; 轮次++) {
			let 函数值, 分组索引;
			if (轮次 < 16) {
				函数值 = (状态B & 状态C) | (~状态B & 状态D);
				分组索引 = 轮次;
			...
			const 新状态B = (状态B + MD5循环左移((状态A + 函数值 + MD5正弦表[轮次] + 分组[分组索引]) | 0, MD5移位表[轮次])) | 0;
			状态A = 状态D;
			状态D = 状态C;
			状态C = 状态B;
			状态B = 新状态B;
	...
	return [初始A, 初始B, 初始C, 初始D].map(状态值 => {
		let 十六进制 = '';
		for (let 偏移 = 0; 偏移 < 4; 偏移++) 十六进制 += ((状态值 >>> (8 * 偏移)) & 0xff).toString(16).padStart(2, '0');
		return 十六进制;
	}).join('');
}

async function MD5MD5(文本) {
	const 第一次十六进制 = MD5十六进制(文本);
	return MD5十六进制(第一次十六进制.slice(7, 27)).toLowerCase();
}
...
async function ShyVPN页面增强(response, env = {}) {
	...
	const panelURL = env.CFST_PANEL_URL || 'https://test.example.com/';
...
async function nginx(env = {}) {
	...
	<p><a class="btn" href="/admin">打开 ShyVPN 设置</a> <a class="btn" href="${env.CFST_PANEL_URL || 'https://test.example.com/'}">打开优选测速面板</a></p>
	</body>
	</html>
	`
}
