...
export default {
	async fetch(request, env, ctx) {
		...
		if (访问路径 === '' && request.method === 'GET' && 是订阅客户端UA(UA, request)) {
			url.pathname = '/sub';
			url.searchParams.set('token', await MD5MD5(host + userID));
			访问路径 = 'sub';
		}
		if (缓存SOCKS5白名单 === null) {
		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			if (url.protocol === 'http:' && 访问路径 !== 'sub') return Response.redirect(url.href.replace(`http://${url.hostname}`, `https://${url.hostname}`), 301);
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'singbox') {
							...
							订阅内容 = await 简化Clash订阅配置(订阅内容, config_JSON);
...
function 重命名Clash节点(节点文本) {
	let 节点名称 = 节点文本.match(/name:\s*([^,\}\n]+)/)?.[1]?.trim()?.replace(/^["']|["']$/g, '') || '';
	const 节点地址 = 节点文本.match(/server:\s*([^,\}\n]+)/)?.[1]?.trim()?.replace(/^["']|["']$/g, '') || '';
	const 节点端口 = 节点文本.match(/port:\s*([^,\}\n]+)/)?.[1]?.trim()?.replace(/^["']|["']$/g, '') || '';
	const 国家 = 节点名称.match(/美国|日本|新加坡|德国|荷兰/)?.[0] || '优选节点';
	const 速度 = 节点名称.match(/\d+(?:\.\d+)?m\/s/i)?.[0] || '';
	const 新名称 = `${国家} | ${节点地址}${节点端口 ? ':' + 节点端口 : ''}${速度 ? ' | ' + 速度 : ''}`;
	if (节点文本.trim().startsWith('- {')) {
		return {
			原名称: 节点名称,
			新名称,
			节点文本: 节点文本.replace(/name:\s*([^,\}\n]+)/, `name: ${YAML字符串值(新名称)}`)
		};
	}
	return {
		原名称: 节点名称,
		新名称,
		节点文本: 节点文本.replace(/^(\s*-\s*name:\s*).+$/m, `$1${YAML字符串值(新名称)}`)
	};
}

function 提取Clash节点(配置文本) {
	const lines = 配置文本.split('\n');
	const 节点 = [];
	let inProxies = false;
	let index = 0;
	while (index < lines.length) {
		...
		if (/^\s*-\s*\{/.test(line)) {
			let block = line, braceCount = (line.match(/\{/g) || []).length - (line.match(/\}/g) || []).length;
			index++;
		...
		if (/^\s*-\s*name:\s*/.test(line)) {
			...
			index++;
			while (index < lines.length && !/^\s*-\s*(?:name:|\{)/.test(lines[index]) && !/^[A-Za-z][\w-]*:\s*/.test(lines[index])) {
				blockLines.push(lines[index]);
				index++;
			}
			节点.push(blockLines.join('\n'));
			continue;
		}
		index++;
	}
...
function 解析速度数值(速度文本) {
	const match = String(速度文本 || '').match(/(\d+(?:\.\d+)?)/);
	return match ? Number(match[1]) : 0;
}

function 解析V2raySSRCFIP表格(html) {
	const rows = [];
	...
	for (const match of String(html || '').matchAll(rowRegex)) {
		rows.push({
			...
			line: String(match.groups.line || '').trim().replace(/^ipv6$/i, 'IPV6'),
			ip: String(match.groups.ip || '').trim(),
			port: 443,
			country: '优选',
			speed: String(match.groups.speed || '').trim().replace(/mb\/s/i, 'm/s'),
			latency: String(match.groups.latency || '').trim(),
			updated: String(match.groups.updated || '').trim(),
		});
...
async function 获取CFIP线路数据(config_JSON = {}) {
	const 自有数据URL = config_JSON?.CFIPDataURL || config_JSON?.优选订阅生成?.CFIPDataURL || null;
	const urls = [
		自有数据URL,
		'https://raw.githubusercontent.com/shiranzby/cmliu-edgetunnel/main/data/cfip.json',
	...
	for (const url of urls) {
		try {
			...
			const parsed = 解析V2raySSRCFIP表格(text);
			if (parsed.length) return parsed;
		} catch (error) {
			console.warn(`[CFIP线路] 获取失败 ${url}: ${error.message}`);
		}
	}
	return [];
}

function 生成CFIP线路Clash节点(基础节点文本, cfipNodes) {
	const 分组 = new Map();
	const 节点块 = [];
	const 已用名称 = new Set();
	const 创建名称 = (node) => {
		...
		while (已用名称.has(name)) name = `${base} #${suffix++}`;
		已用名称.add(name);
		return name;
	};
	for (const node of cfipNodes || []) {
		const ip = String(node.ip || node.server || '').trim();
		if (!ip || ip.includes(':')) continue;
		const line = String(node.line || '优选线路').trim() || '优选线路';
		const port = String(node.port || 443);
		const name = 创建名称(node);
		let text = 基础节点文本;
		if (text.trim().startsWith('- {')) {
			...
			text = text
				.replace(/^(\s*-\s*name:\s*).+$/m, `$1${YAML字符串值(name)}`)
				.replace(/^(\s*server:\s*).+$/m, `$1${YAML字符串值(ip)}`)
				.replace(/^(\s*port:\s*).+$/m, `$1${port}`);
		}
		节点块.push(text);
		if (!分组.has(line)) 分组.set(line, []);
		分组.get(line).push(name);
	}
	return { 节点块, 分组 };
}

async function 简化Clash订阅配置(配置文本, config_JSON = {}) {
	const 原节点列表 = 提取Clash节点(配置文本).filter(节点 => /type:\s*(?:vless|trojan|ss|vmess|hysteria2?|tuic|socks5|http)/i.test(节点));
	if (!原节点列表.length) return 配置文本;
	const 重命名结果 = 原节点列表.map(节点 => 重命名Clash节点(节点));
	const 节点名称列表 = 重命名结果.map(item => item.新名称);
	const cfip结果 = 生成CFIP线路Clash节点(重命名结果[0].节点文本, await 获取CFIP线路数据(config_JSON));
	const 节点块 = 重命名结果.map(item => item.节点文本).concat(cfip结果.节点块).join('\n');
	const 分组节点 = 节点名称列表.map(name => `      - ${YAML字符串值(name)}`).join('\n');
	const 线路分组 = [...cfip结果.分组.entries()].map(([line, names]) => `  - name: ${YAML字符串值(line)}
    type: url-test
    url: http://www.gstatic.com/generate_204
    interval: 300
    tolerance: 50
    proxies:
${names.map(name => `      - ${YAML字符串值(name)}`).join('\n')}`).join('\n');
...
function 是订阅客户端UA(UA, request) {
	const ua = (UA || '').toLowerCase();
	if (request.headers.get('subconverter-request') || request.headers.get('subconverter-version')) return true;
	if (!ua || ua === 'null' || ua.includes('mozilla')) return false;
	return ['sparkle', 'mihomo', 'clash', 'meta', 'sing-box', 'singbox', 'v2ray', 'v2rayn', 'nekoray', 'stash', 'surge', 'loon', 'quantumult', 'subconverter'].some(关键词 => ua.includes(关键词));
}
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	const host = hostname, Ali_DoH = "https://dns.alidns.com/dns-query", ECH_SNI = "cloudflare-ech.com", 占位符 = '{{IP:PORT}}', 初始化开始时间 = performance.now(), 默认配置JSON = {
		...
		优选订阅生成: {
			...
			CFIPDataURL: env.CFIP_DATA_URL || 'https://raw.githubusercontent.com/shiranzby/edgetunnel/main/data/cfip.json',
			本地IP库: {
	...
	if (!config_JSON.优选订阅生成.CFIPDataURL) config_JSON.优选订阅生成.CFIPDataURL = env.CFIP_DATA_URL || 'https://raw.githubusercontent.com/shiranzby/edgetunnel/main/data/cfip.json';
...
async function 生成随机IP(request, count = 16, 指定端口 = -1) {
	const 目标国家 = ['us', 'jp', 'sg', 'de', 'nl'];
	const 国家名称映射 = { us: '美国', jp: '日本', sg: '新加坡', de: '德国', nl: '荷兰' };
	const 内置优选数据 = `104.16.94.26#us 美国 US 18.27m/s
172.64.176.131#us 美国 US 15.84m/s
104.18.2.29#us 美国 US 15.63m/s
104.17.1.194#us 美国 US 13.79m/s
104.16.2.113#us 美国 US 13.72m/s
198.41.207.34#us 美国 US 12.86m/s
162.159.38.118#jp 日本 JP 24.18m/s
172.64.229.156#jp 日本 JP 23.0m/s
162.159.39.16#jp 日本 JP 22.55m/s
108.162.198.70#jp 日本 JP 21.56m/s
172.64.230.154#jp 日本 JP 14.72m/s
172.64.231.160#jp 日本 JP 14.51m/s
198.41.223.110#sg 新加坡 SG 22.24m/s
162.159.9.209#sg 新加坡 SG 22.01m/s
162.159.2.42#sg 新加坡 SG 21.3m/s
108.162.195.19#sg 新加坡 SG 21.17m/s
162.159.11.2#sg 新加坡 SG 20.82m/s
162.159.10.207#sg 新加坡 SG 20.21m/s
173.245.58.199#sg 新加坡 SG 19.53m/s
172.64.35.87#sg 新加坡 SG 16.74m/s
104.25.0.89#de 德国 DE 19.53m/s
104.27.2.4#de 德国 DE 17.95m/s
104.27.1.111#de 德国 DE 17.11m/s
104.27.3.200#de 德国 DE 14.0m/s
104.26.0.211#de 德国 DE 12.95m/s
104.25.1.172#de 德国 DE 11.8m/s
104.25.3.1#de 德国 DE 11.68m/s
188.114.97.3#nl 荷兰 NL 17.9m/s
104.20.2.70#nl 荷兰 NL 16.2m/s
188.114.99.177#nl 荷兰 NL 15.21m/s
104.20.3.208#nl 荷兰 NL 15.19m/s
104.20.1.143#nl 荷兰 NL 14.7m/s
188.114.98.80#nl 荷兰 NL 14.32m/s
188.114.96.226#nl 荷兰 NL 13.54m/s
104.20.0.216#nl 荷兰 NL 11.75m/s`;
	const 解析优选行 = (line) => {
		const match = String(line || '').trim().match(/^([\d.]+)(?::(\d+))?#([a-z]{2})\s+(.+?)\s+[A-Z]{2}(?:\s+(\d+(?:\.\d+)?m\/s))?/i);
		if (!match || !目标国家.includes(match[3].toLowerCase())) return null;
		const code = match[3].toLowerCase();
		return {
			ip: match[1],
			port: match[2] || '443',
			country: 国家名称映射[code] || match[4],
			speed: match[5] || '',
			speedValue: match[5] ? Number(match[5].replace(/m\/s/i, '')) : 0,
		};
	};
	const 从文本生成优选 = (text) => {
		const 优选结果 = String(text || '').split(/\r?\n/)
			.map(解析优选行)
			.filter(Boolean)
			.sort((a, b) => b.speedValue - a.speedValue);
		const 按国家 = new Map();
		for (const item of 优选结果) {
			const code = Object.entries(国家名称映射).find(([, name]) => name === item.country)?.[0] || item.country;
			if (!按国家.has(code)) 按国家.set(code, []);
			按国家.get(code).push(item);
		}
		const selected = [];
		while (selected.length < count && selected.length < 优选结果.length) {
			for (const code of 目标国家) {
				const item = 按国家.get(code)?.shift();
				if (item) selected.push(item);
				if (selected.length >= count) break;
			}
		}
		return selected.slice(0, count).map(item => {
			const port = 指定端口 === -1 ? item.port : String(指定端口);
			return `${item.ip}:${port}#${item.country}${item.speed ? ' ' + item.speed : ''}`;
		});
	};
	try {
		const response = await fetch('https://raw.githubusercontent.com/gslege/CloudflareIP/main/country.txt', { headers: { 'User-Agent': 'edge' + 'tunnel CFIP selector' } });
		if (response.ok) {
			const 优选IP = 从文本生成优选(await response.text());
			if (优选IP.length) {
				return [优选IP, 优选IP.join('\n')];
			}
		}
	} catch (error) {
		console.warn(`[优选IP] gslege/CloudflareIP 获取失败，使用内置优选数据: ${error.message}`);
	}

	const 备用IP = 从文本生成优选(内置优选数据);
	return [备用IP, 备用IP.join('\n')];
}
